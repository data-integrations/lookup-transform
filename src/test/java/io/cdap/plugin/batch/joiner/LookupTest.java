...
public class LookupTest extends ETLBatchTestBase {
  ...
  private void joinHelper(ETLStage customerStage, ETLStage phoneNumbersStage, ETLStage joinStage,
                          ...
                          BiFunction<Schema, Table, Void> verifyOutput,
    ...
    DataSetManager<Table> outputManager = getDataset(joinedDatasetName);
    Table table = outputManager.get();

    // verify join output
    verifyOutput.apply(outputSchema, table);
  }
  ...
  public void testLookup() throws Exception {
    ...
    final ImmutableMap<String, String> configProperties = new ImmutableMap.Builder<String, String>()
      ...
      .build();
    ETLStage joinStage = new ETLStage("joiner", new ETLPlugin("Lookup", Lookup.PLUGIN_TYPE, configProperties));
    ETLStage joinSinkStage = new ETLStage("sink", MockSink.getPlugin(JOINED_DATASET_NAME));
